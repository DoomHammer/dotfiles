...
let
  ...
  dir = "${flakePath config}/home-manager/features/shell";
in
{
# Enable dircolors?
...
# Port settings to https://nix-community.github.io/home-manager/options.xhtml#opt-programs.starship.settings
  ...
  home.file = {
    ".shell-aliases".source =
    config.lib.file.mkOutOfStoreSymlink "${dir}/shell-config/shell-aliases";
    ".shell-common".source =
    config.lib.file.mkOutOfStoreSymlink "${dir}/shell-config/shell-common";
    ".shell-env".source =
    config.lib.file.mkOutOfStoreSymlink "${dir}/shell-config/shell-env";
    ".shell-logout".source =
    config.lib.file.mkOutOfStoreSymlink "${dir}/shell-config/shell-logout";
  };
}
